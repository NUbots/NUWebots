...
using namespace controller::nugus;
...
class PlayerServer {
public:
    PlayerServer(const std::vector<std::string>& allowed_hosts,
                 int port,
                 int player_id,
                 int team,
                 webots::Supervisor* robot, std::vector<webots::Node*> nodes)
        ...
        switch (device->getNodeType()) {
            ...
            default:
                warn(sensor_measurements,
                     "Device \"" + device->getName() + "\" is not supported, time step command, ignored.");
        }
    }
    ...
    void processBuffer() {
        ...
        for (int i = 0; i < actuatorRequests.motor_positions_size(); i++) {
            ...
            if (motor) {
                ...
                warn(sensor_measurements,
                     "Motor \"" + motorPosition.name() + "\" not found, position command ignored.");
        }
        for (int i = 0; i < actuatorRequests.motor_velocities_size(); i++) {
            ...
            if (motor) {
                ...
                warn(sensor_measurements,
                     "Motor \"" + motorVelocity.name() + "\" not found, velocity command ignored.");
        }
        ...
        for (int i = 0; i < actuatorRequests.camera_qualities_size(); i++) {
            ...
            if (camera)
                ...
                warn(sensor_measurements,
                     "Camera \"" + cameraQuality.name() + "\" not found, quality command ignored.");
        }
        for (int i = 0; i < actuatorRequests.camera_exposures_size(); i++) {
            ...
            if (camera)
                ...
                warn(sensor_measurements,
                     "Camera \"" + cameraExposure.name() + "\" not found, exposure command ignored.");
        }
        ...
        for (int i = 0; i < actuatorRequests.sensor_time_steps_size(); i++) {
            ...
            if (device) {
                ...
                if (sensor_time_step == 0)
                    ...
                    warn(sensor_measurements,
                         "Time step for \"" + sensorTimeStep.name() + "\" should be greater or equal to "
                             + std::to_string(min_time_step) + ", ignoring " + std::to_string(sensor_time_step)
                             + " value.");
                else if (sensor_time_step % basic_time_step != 0)
                    warn(sensor_measurements,
                         "Time step for \"" + sensorTimeStep.name() + "\" should be a multiple of "
                    ...
                    for (const auto& entry : sensors)
                        if (entry.first->getNodeType() == webots::Node::CAMERA
                            && entry.first->getName() != sensorTimeStep.name())
                            requested_bandwidth +=
                                getRenderingBandwidth(static_cast<webots::Camera*>(entry.first), entry.second);
                    ...
                    if (requested_bandwidth > robot_rendering_quota)
                        warn(sensor_measurements,
                             "requested rendering bandwidth is above the limit ("
    ...
    void prepareSensorMessage() {
        ...
        for (const auto& node : robot_nodes) {
            const double* rTXx = node->getField("translation")->getSFVec3f();
            fvec3* rrff = localisation_ground_truth->add_rrff();
            rrff->set_x(rTXx[0]);
            rrff->set_y(rTXx[1]);
            rrff->set_z(rTXx[2]);
...
int main(int argc, char* argv[]) {
    ...
    PlayerServer server(allowed_hosts, port, player_id, player_team, robot, robot_nodes);
